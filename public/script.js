...
const appendMessage = (userType, message, msgPos = "") =>{
  ...
  if(msgPos == "isFirst"){
    ...
    <img src="./pizza.jpg" class="shadow-xs img-fluid rounded-top" alt="pizza">
    ...
    // msg_wrapper.classList.add(classN)
    msg_wrapper.innerHTML = `
        <h6 class="msg_wrapper_tsitle fw-bold text-black">Order 👇</h6>
        
        <p class="btn menu_response active rounded bg-light border-0 m-1 outline-0 fw-400 btn-sm px-4 py-2"> ${message.replace('Order ', "")} </p>
        <div class="menus">
    ...
    welcomeMessageElement.appendChild(msg_wrapper);
     ...
     msg_wrapper.innerHTML = `
          <img src="./pizza.jpg" class="shadow-xs img-fluid rounded-top" alt="pizza">
         <h6 class="msg_wrapper_tsitle fw-bold text-black">Order Completed</h6>
    ...
    // msg_wrapper.classList.add(classN)
    msg_wrapper.innerHTML = `<img src="./pizza.jpg" class="shadow-xs img-fluid rounded-top" alt="pizza">
        <h6 class="msg_wrapper_tsitle fw-bold text-black">Order Completed</h6>
    ...
    const msg_wrapper = document.createElement("div");
    msg_wrapper.className = "msg_wrapper text-dark d-flex justify-content-end align-items-center p-2" + classN
...
const appendButton = ( itemList, menu, msgPos = "" ) => {
  ...
  const buttons = itemList.map((item) => {
    ...
    if(msgPos == "menu"){
      button.className = 'menu_btn rounded bg-light border-0 m-1 outline-0 fw-400 btn-sm px-4 py-2'
    }
...
const cardContainer = ( list ) => {
    ...
    orderHistoryHeader.innerHTML = `<img src="./pizza.jpg" class="shadow-xs img-fluid rounded-top" alt="pizza">
    ...
    for (let order of list ) {
        ...
        orderItem.innerHTML = `<div>Item: ${ order.itemname } </div> <div class='d-flex justify-content-end'> <small class='badge btn-sm ${ order.status == 'completed' ? 'bg-success' : (order.status == 'cancelled' ? 'bg-dark' : 'bg-danger' ) }'>${ order.status }</small></div>`
...
socket.on("completed_response", function( msg ){
  setTimeout(() => {
      appendMessage(true, msg, "completed" )
        ...
        let buttonContainer = document.getElementById(`completed_button-container${tries}`)
        buttonContainer.innerHTML = ''
        const button1 = button('Back', { key: '88', value: 'Back' })
        button1.className = 'btn btn-outline-dark w-100 mb-1'
        buttonContainer.appendChild( button1 )
...
socket.on("menu_response", function( msg ){
    setTimeout(() => {
        ...
        let buttonContainer = document.getElementById(`menu_response_btn_wrapper${tries}`)
        buttonContainer.innerHTML = ''
        ...
        if(msg.includes("placed any order yet")){
          buttonContainer.appendChild( button3)
        }else{
          buttonContainer.appendChild(button1)
          buttonContainer.appendChild(button2)
